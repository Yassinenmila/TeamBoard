      ...
      <header class="h-24 px-12 flex items-center justify-between shrink-0 bg-white border-b border-slate-100">
        ...
        <div class="flex items-center gap-6">
          ...
          <button
            ...
            :disabled="loading || (form.status === 'rejected' && !form.commentaire_responsable)"
            ...
            <div class="bg-white p-10 rounded-[2.5rem] border border-slate-100 shadow-sm space-y-10">
              ...
              <div class="flex items-center gap-6 p-6 bg-slate-50 rounded-[2rem] border border-slate-100">
                <div class="w-16 h-16 bg-white rounded-2xl shadow-sm flex items-center justify-center text-xl font-black italic text-slate-900">
                  {{ demande.user?.first_name ? demande.user.first_name[0] : 'U' }}
                </div>
                <div>
              ...
              <div>
                ...
                <div class="w-full bg-slate-50 rounded-[2rem] p-8 text-sm font-bold text-slate-900 border border-transparent italic leading-relaxed whitespace-pre-line">
                  {{ demande.description }}
                </div>
              </div>
            ...
            <div class="bg-slate-900 p-8 rounded-[2.5rem] shadow-2xl shadow-slate-900/20 text-white space-y-8">
              ...
              <div class="space-y-3">
                ...
                <div class="grid grid-cols-1 gap-2">
                  <button
                    ...
                    :class="form.status === key ? statusThemes[key].active : 'bg-slate-800 text-slate-500 border-transparent'"
              ...
              <div>
                <label class="block text-[9px] font-bold text-slate-500 uppercase tracking-widest mb-4 italic">
                  Justification
                  <span v-if="form.status === 'rejected'" class="text-rose-500 font-black tracking-tighter ml-2">(Obligatoire pour refus)</span>
                </label>
                <textarea
                  v-model="form.commentaire_responsable"
                  rows="5"
...
// Synchronisé avec tes précédents composants (status et commentaire_responsable)
const form = reactive({
  status: 'pending',
...
const statusThemes = {
  pending: { icon: '⏳', active: 'bg-amber-500/10 border-amber-500/50 text-amber-500 shadow-lg shadow-amber-500/5' },
  accepted: { icon: '✅', active: 'bg-emerald-500 text-white border-emerald-400 shadow-lg shadow-emerald-500/20' },
  rejected: { icon: '❌', active: 'bg-rose-600 text-white border-rose-400 shadow-lg shadow-rose-600/20' }
};
...
const fetchDemande = async () => {
  try {
    ...
    // Normalisation du statut venant de l'API
    const apiStatus = demande.value.status || demande.value.statut;
    if (apiStatus === 'valide' || apiStatus === 'acceptee') form.status = 'accepted';
    else if (apiStatus === 'rejete' || apiStatus === 'refusee') form.status = 'rejected';
    else form.status = 'pending';

    form.commentaire_responsable = demande.value.commentaire_responsable || '';
  } catch (error) {
    console.error(error);
...
const submitDecision = async () => {
  loading.value = true;
  try {
    // Utilisation de PUT /api/demandes/{id}/status ou decision selon ton backend
    await api.post(`/demandes/${demande.value.id}/traiter`, {
      statut: form.status,
      contenu: form.commentaire_responsable
    });
    router.push('/admin/demandes');
  } catch (error) {
...
const goBack = () => router.push('/admin/demandes');
